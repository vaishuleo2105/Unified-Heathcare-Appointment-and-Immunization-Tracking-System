...
const STEP_LABELS = ['Visit Type', 'Doctor & Slot', 'Confirm']
...
export default function BookAppointmentModal({ onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    setSlots([]); setSelectedSlot(''); setSlotError(''); setLoadingSlots(true)
    api.get(`/doctors/${selectedDoctor._id}/slots`, { params: { date } })
      .then(({ data }) => {
        if (data.slots.length === 0) setSlotError('No available slots on this date. Please choose another date.')
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true)
    try {
      await api.post('/appointments', { doctorId: selectedDoctor._id, date, time: selectedSlot, type: visitType, notes })
  ...
  function canProceed() {
    ...
    if (step === 2) return !!selectedDoctor && !!selectedSlot
    return true
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[92vh] flex flex-col">
              ...
              <div key={i} className="flex items-center gap-1 flex-1">
                <div className="flex flex-col items-center flex-1">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                    step > i + 1 ? 'bg-secondary text-white' : step === i + 1 ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant'
                  }`}>
                    {step > i + 1 ? <span className="material-symbols-outlined text-sm">check</span> : i + 1}
                  </div>
                  <span className={`text-[10px] font-semibold mt-1 text-center ${step === i + 1 ? 'text-primary' : 'text-on-surface-variant'}`}>{label}</span>
                </div>
                {i < STEP_LABELS.length - 1 && <div className={`h-0.5 flex-1 mb-4 transition-all ${step > i + 1 ? 'bg-secondary' : 'bg-outline-variant'}`} />}
              </div>
            ))}
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {VISIT_TYPES.map(v => (
                  <button key={v.value} onClick={() => setVisitType(v.value)}
                    className={`p-4 rounded-xl border-2 text-left transition-all hover:border-primary ${visitType === v.value ? 'border-primary bg-primary-fixed' : 'border-outline-variant hover:bg-surface-container-low'}`}>
                    <span className={`material-symbols-outlined text-2xl mb-2 block ${visitType === v.value ? 'text-primary' : 'text-on-surface-variant'}`}>{v.icon}</span>
            ...
            <div>
              ...
              {loadingDoctors ? (
                ...
                <div className="text-center py-10 bg-surface-container-low rounded-xl">
                  ...
                  <p className="text-sm text-on-surface-variant mt-2">No doctors available currently.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="space-y-3">
                    {doctors.map(doc => (
                      <button key={doc._id} onClick={() => { setSelectedDoctor(doc); setDate(''); setSlots([]); setSelectedSlot('') }}
                        className={`w-full text-left p-4 rounded-xl border-2 transition-all ${selectedDoctor?._id === doc._id ? 'border-primary bg-primary-fixed' : 'border-outline-variant hover:border-primary hover:bg-surface-container-low'}`}>
                        <div className="flex items-center gap-3">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${selectedDoctor?._id === doc._id ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant'}`}>
                            <span className="material-symbols-outlined text-2xl">medical_services</span>
                          </div>
                          <div className="flex-1">
                            <p className="text-sm font-bold text-on-surface">Dr. {doc.firstName} {doc.lastName}</p>
                            <p className="text-xs text-on-surface-variant">{doc.email}</p>
                          </div>
                          {selectedDoctor?._id === doc._id && <span className="material-symbols-outlined text-primary text-xl">check_circle</span>}
                        </div>
                      </button>
                    ))}
                  </div>
                  ...
                  {selectedDoctor && (
                    <div className="mt-4 p-4 bg-surface-container-low rounded-xl space-y-4">
                      <div>
                        <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2 block">Select Date</label>
                      ...
                      {date && (
                        <div>
                          <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2 block">Available Time Slots</label>
                          {loadingSlots ? (
                            ...
                            <div className="flex items-center gap-2 bg-error-container text-on-error-container px-4 py-3 rounded-xl text-sm">
                              <span className="material-symbols-outlined text-base">event_busy</span>{slotError}
                            </div>
                          ) : (
                            <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                              {slots.map(slot => (
                                <button key={slot} onClick={() => setSelectedSlot(slot)}
                                  className={`py-2.5 text-xs font-semibold rounded-xl border-2 transition-all ${selectedSlot === slot ? 'border-primary bg-primary text-white' : 'border-outline-variant bg-white hover:border-primary hover:bg-primary-fixed text-on-surface'}`}>
                                  {slot}
                                </button>
                              ))}
                            </div>
            ...
            <div>
              ...
              <div className="bg-surface-container-low rounded-xl p-5 space-y-3 mb-5">
                {[
                  ...
                  { icon: 'medical_services', label: 'Doctor', value: `Dr. ${selectedDoctor?.firstName} ${selectedDoctor?.lastName}` },
              ...
              <div>
                <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2 block">Symptoms / Notes (Optional)</label>
                <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Describe your symptoms or reason for visit..." rows={3}
        ...
        <div className="flex gap-3 p-6 border-t border-outline-variant flex-shrink-0">
          {step > 1 ? (
            <button onClick={() => setStep(s => s - 1)} className="flex-1 py-3 border border-outline-variant text-on-surface text-sm font-semibold rounded-xl hover:bg-surface-container transition-colors">Back</button>
          ) : (
            <button onClick={onClose} className="flex-1 py-3 border border-outline-variant text-on-surface text-sm font-semibold rounded-xl hover:bg-surface-container transition-colors">Cancel</button>
          )}
          {step < 3 ? (
            ...
            <button onClick={handleSubmit} disabled={submitting || !!success}
              ...
              {submitting ? (
                ...
                <><span className="material-symbols-outlined text-xl">check_circle</span>Confirm Booking</>
              )}
            </button>
          )}
        </div>
